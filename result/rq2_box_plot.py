...
fileName = os.path.join(base_path, "result", "rq2" + ".csv")
...
if mode == 'train':
    cols = ['Scratch', 'Update', 'Stacking']
    df.rename(columns={
        'MU Train': 'Update',
        'MS Train': 'Stacking',
        'Scratch Train': 'Scratch'
    }, inplace=True)
else:
    cols = ['Scratch','Voting-only', 'SC+Voting']
    df.rename(columns={
        'Voting Infer': 'Voting-only',
        'SC Infer': 'SC+Voting',
        'Scratch Infer': 'Scratch'
    }, inplace=True)
