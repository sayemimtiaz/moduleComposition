...
from evaluation.accuracy_computer import getModulePredictionAnyToManyUnrolled, getModulePredictionAnyToOneUnrolled, \
...
module_path1 = os.path.join(base_path, activation, 'many_to_many_same', 'modules', modelName)
module_path2 = os.path.join(base_path, activation, 'many_to_one', 'modules', modelName)

scratch_model_path = os.path.join(base_path, activation, 'many_to_one', 'h5', modelName + '_scratch.h5')
model_path1 = os.path.join(base_path, activation, 'many_to_many_same', 'h5', modelName + '.h5')
model_path2 = os.path.join(base_path, activation, 'many_to_one', 'h5', modelName + '.h5')
...
x_train1, x_test1, y_train1, y_test1, num_words1, timestep1, nb_classes1 = \
    load_pos_tagged_dataset_with_clinc(hot_encode=False)

x_train2, x_test2, y_train2, y_test2, num_words2, \
timestep2, nb_classes2 = loadClincOosWithPosTag(hot_encode=False, timestep=15)
...
for m in range(nb_classes2):
    modularLayers = initModularLayers(model2.layers)
    repopulateModularWeights(modularLayers, module_path2, m)
    modules2[m]=modularLayers
    # modules2[m] = load_model(os.path.join(module_path2, 'module' + str(m) + '.h5'))
...
for class1 in range(nb_classes1):
    ...
    for class2 in range(nb_classes2):
        ...
        predClass1Masked, predClass1Unmasked = \
            getModulePredictionAnyToManyUnrolled(moduleClass1, xt, yt, timestep1)
        # predClass2 = moduleClass2.predict(xt)
        predClass2 = getModulePredictionAnyToOneUnrolled(moduleClass2, xt, yt, moduleNo=class2)

        # predClass2Masked, predClass2Unmasked = \
        #     getModulePredictionAnyToOneUnrolledWithMasked(moduleClass2, xt, yt)

        finalPred = []
        for i in range(0, len(yt)):

            # if class1 in predClass1Unmasked[i] and class2 != predClass2[i].argmax():
            #     finalPred.append(0)
            # elif class2 == predClass2[i].argmax() and \
            #         class1 not in predClass1Unmasked[i]:
            #     finalPred.append(1)
            # else:
            temp_c1 = []
            ...
            # temp_c1_no = predClass1Masked[i][temp_c1.argmax()][0]
            temp_c1 = temp_c1.max()
            # temp_c1 = temp_c1 / temp_c1_no

            if class2 != 0:
                # temp_c2_no = predClass2Masked[i][0]
                temp_c2_no = predClass2[i][0]
            else:
                # temp_c2_no = predClass2Masked[i][1]
                temp_c2_no = predClass2[i][1]

            # temp_c2 = predClass2Masked[i][class2]
